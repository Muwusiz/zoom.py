# zoom.sh converted to work with windows 10 by Musiz#1215
...
system(f'py generate.py -p="{TEXT}" -opt="{OPTIMISER}" -lr={LR} -i={MAX_ITERATIONS} -se={MAX_ITERATIONS} --seed={SEED} -o="Zoom/{FILENAME_NO_EXT}/{FILENAME}"')
...
for i in range(int(MAX_EPOCHS)):
    ...
    system(f'py generate.py -p="{TEXT}" -opt="{OPTIMISER}" -lr={LR} -i={MAX_ITERATIONS} -se={MAX_ITERATIONS} --seed={SEED} -ii="Zoom/{FILENAME_NO_EXT}/{FILENAME}" -o="Zoom/{FILENAME_NO_EXT}/{FILENAME}"')
